<?php
/**
 * 著者データ [ M_MX332_01_20170527050736.dat ]
 * 次の2カラムからなるtsv区切りで出力
 * |品目コード|著者名|
...
$fp = fopen("M_MX332_01_20170527050736.dat", "r");
$fp_out1 = fopen("writer_alpha_no_slash.txt", "w");
$fp_out2 = fopen("writer_alpha_exist_slash.txt", "w");
while (($buffer = fgets($fp, 4096)) !== false) {
	...
	if ($exist) {
		echo "$hinmoku, $choshamei \n";
		$exist = exist_slash($choshamei);
		if ($exist) {
			fputs($fp_out2, "$hinmoku\t$choshamei".chr(10));
		} else {
			fputs($fp_out1, "$hinmoku\t$choshamei".chr(10));
		}
	}
}
fclose($fp);
